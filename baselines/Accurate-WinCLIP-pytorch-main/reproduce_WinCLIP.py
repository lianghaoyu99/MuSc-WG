...
def test(args,):
    ...
    for index, items  in enumerate(tqdm(test_dataloader)):
        ...
        # Determine the correct index to use for features
        if args.class_name != "all":
            cls_id = [0] * len(cls_name)  # Since obj_list only has 1 item, its index is always 0
        else:
            cls_id = items['cls_id']
    ...
    efficiency_info = 'WinCLIP: {:.2f}ms per image'.format((total_time) * 1000 / total_images)
    print(efficiency_info)
    logger.info(efficiency_info)
    if torch.cuda.is_available() and 'cuda' in device:
        gpu_info = 'WinCLIP GPU Memory: {:.2f} MB (Allocated), {:.2f} MB (Reserved)'.format(
