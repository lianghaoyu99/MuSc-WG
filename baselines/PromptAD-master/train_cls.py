...
def fit(model,
    ...
    epoch_pbar = tqdm(range(args.Epoch), desc=f"Training [{args.class_name}]", leave=True)
    for epoch in epoch_pbar:
        ...
        # Update description for test phase
        epoch_pbar.set_description(f"Testing [{args.class_name}] Epoch {epoch+1}/{args.Epoch}")
        ...
        if best_result_dict is None:
            ...
            if args.vis:
                plot_sample_cv2(names, test_imgs, {'PromptAD': np.array(score_maps)}, gt_mask_list, save_folder=args.img_dir)
        ...
        # Display current best I-AUROC on the progress bar
        epoch_pbar.set_postfix({'Best I-ROC': f"{best_result_dict['i_roc']:.4f}", 'Cur I-ROC': f"{result_dict['i_roc']:.4f}"})
...
def main(args):
    ...
    args.img_dir = img_dir  # Save img_dir to args so we can use it in fit()
...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='mvtec', choices=['mvtec', 'visa', 'microled', 'miniled'])
