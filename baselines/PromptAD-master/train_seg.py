...
def fit(model,
    ...
    epoch_pbar = tqdm(range(args.Epoch), desc=f"Training SEG [{args.class_name}]", leave=True)
    for epoch in epoch_pbar:
        ...
        epoch_pbar.set_description(f"Testing SEG [{args.class_name}] Epoch {epoch+1}/{args.Epoch}")
        ...
        # Display current best P-AUROC on the progress bar
        epoch_pbar.set_postfix({'Best P-ROC': f"{best_result_dict['p_roc']:.4f}", 'Cur P-ROC': f"{result_dict['p_roc']:.4f}"})
