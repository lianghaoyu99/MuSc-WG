...
class FSAD_Dataset_train(Dataset):
    def __init__(self,
        ...
        assert class_name in CLASS_NAMES or 'led' in class_name.lower(), 'class_name: {}, should be in {}'.format(class_name, CLASS_NAMES)
    ...
    def shuffle_dataset(self):
        ...
        data_img = {}
        classes_to_load = CLASS_NAMES if 'led' not in self.class_name.lower() else [d for d in os.listdir(self.dataset_path) if os.path.isdir(os.path.join(self.dataset_path, d))]
        for class_name_one in classes_to_load:
...
class FSAD_Dataset_test(Dataset):
    ...
    def load_dataset_folder(self):
        ...
        # few-shot deterministic sampling from train/good
        if self.shot > 0:
        ...
        for img_type in data_img.keys():
            for image_dir_one in data_img[img_type]:
                ...
                # Check for LED mask format vs MVTec mask format
                is_led = 'led' in self.class_name.lower()
                query_mask_dir = image_dir_one.replace('test', 'ground_truth')
                if is_led:
                    pass # LED dataset uses exact same filename for mask
                else:
