...
if __name__ == "__main__":
    args = get_args()  # 加载命令行参数
    cfg = load_yaml(args.config)  # 加载配置文件
    cfg = load_args(cfg, args)  # 根据命令行参数更新配置文件
    print(cfg)
    seed = 42
    model = MuSc(cfg, seed=seed)  # 实例化MuSc类，初始化实例参数
    model.main()  # 运行MuSc主代码
